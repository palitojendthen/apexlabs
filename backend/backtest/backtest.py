...
# # ========== CONFIG ==========
# OVERLAY_INDICATORS = {"SMA", "EMA", "PMA", "KAMA", "DEMA", "TEMA"}  # extend easily later
...
#     indicator_cols = []
#     signal_cols = []
#     plots = []  # overlay info for frontend
...
#         if source_key_name in params:
#             source_val = params.pop(source_key_name)
#         elif "source" in params:
#             source_val = params.pop("source")
#         else:
#             source_val = "close"
...
#         # Only the first overlay-type indicator should be plotted
...
#             y_mark = float(df.loc[i, "low"]) * 0.995
#             long_markers.append({"x": str(df.loc[i, "open_time"]), "y": y_mark})
...
#                 trades[-1]["pnl"] = -sl_pct
#                 y_mark = float(df.loc[i, "high"]) * 1.005
#                 short_markers.append({"x": str(df.loc[i, "open_time"]), "y": y_mark})
#                 entry_price = None
...
def main():
    ...
    # df = df.dropna(subset=['open_time', 'open', 'high', 'low', 'close'], axis=0)
    # df = df.dropna(axis=0)
