...
//   // 🔹 helper: store Firebase token to server-side cookie
...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 2️⃣ Fetch and send Firebase token to backend (for UID consistency)
      ...
      // 3️⃣ Set secure cookie for backend auth
      await setSessionCookie();

      // 4️⃣ Redirect to backtest page
      router.push("/");
    } catch (err) {
      console.error("Login failed:", err);
      alert("Login failed. Please check your credentials.");
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 1️⃣ Fetch and send Firebase token to backend
      const token = await user.getIdToken(true);
      await fetch("/api/session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
