...
export default function Home() {
  ...
  const [chartData, setChartData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [backendResult, setBackendResult] = useState(null); // stores backend output
  ...
  const equity = backendResult?.df?.map((d) => ({
    x: d.open_time,
    y: d.equity ?? null,
  })) ?? [];
  ...
  return (
    <main className="min-h-screen pt-24 px-6 bg-white text-black dark:bg-gradient-to-b dark:from-black dark:to-slate-950 dark:text-white transition-colors duration-300 opacity-80">
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl font-semibold mb-2">Research & Backtest</h1>
        <p className="text-neutral-400">Explore strategies, analyze performance, and visualize results.</p>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row gap-10 items-stretch justify-between">
        ...
        <div className="flex-1 max-w-[420px] dark:bg-white/5 rounded-2xl p-6 backdrop-blur-md border dark:border-white/10 flex flex-col">
          <h2 className="text-lg font-semibold mb-4">Configuration</h2>
          <form className="flex flex-col gap-4 flex-grow" onSubmit={handleRun}>
            <input name="symbol" defaultValue={formState.symbol} placeholder="Symbol (e.g. BTCUSDT)" className="p-2 rounded bg-white border border-gray-300 text-gray-800 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white" required />
            <select name="timeframe" defaultValue={formState.timeframe} className="p-2 rounded bg-white border border-gray-300 text-gray-800 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white">
              <option value="1d">1D</option>
              <option value="12h">12H</option>
              <option value="1w">1W</option>
            </select>
            <select name="ind1" defaultValue={formState.ind1} className="p-2 rounded bg-white border border-gray-300 text-gray-800 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white">
              <option>SMA</option>
              <option>EMA</option>
              <option>ADX</option>
              <option>RSI</option>
            ...
            {!chartData ? (
              ...
              <Plot
                data={[
                  ...
                  // // 🔹 Overlay indicators (color coded)
                  // ...(plots?.flatMap((p) => [
                  //   {
                  //     x: chartData.map((d) => d.open_time),
                  //     y: chartData.map((d) => (d[p.signal_col] === 1 ? d[p.name] : null)),
                  //     type: "scatter",
                  //     mode: "lines",
                  //     line: { width: 1.6, color: p.color_up },
                  //     name: `${p.name.toUpperCase()} (Long)`,
                  //   },
                  //   {
                  //     x: chartData.map((d) => d.open_time),
                  //     y: chartData.map((d) => (d[p.signal_col] === 0 ? d[p.name] : null)),
                  //     type: "scatter",
                  //     mode: "lines",
                  //     line: { width: 1.6, color: p.color_down },
              ...
              ...(backendResult?.plots?.length
                ...
                layout={{
                  ...
                  paper_bgcolor: "transparent",
                  plot_bgcolor: "transparent",
                  xaxis: { gridcolor: gridColor, linecolor: borderColor, rangeslider: { visible: false } },
